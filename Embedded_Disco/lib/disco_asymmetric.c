...
/*TO BE REMOVED, I put this here to avoid an error that I got while compiling, used to be in the .h file as inline*/
...
void disco_Initialize(handshakeState *hs, const char *handshake_pattern,
  ...
  if(!pq){
      ...
      if (pq_rs != NULL) {
          ...
          hs->pq_rs.isSet = false;
      }
      if (re != NULL) {
          hs->pq_re = *pq_re;
          hs->pq_re.isSet = true;
      } else {
          hs->pq_re.isSet = false;
      }
  }
    ...
    switch (*handshake_pattern) {
      case token_s:
          if(!pq) {
              ...
              if ((initiator && direction) || (!initiator && !direction)) {
                  mixHash(&(hs->symmetric_state), hs->pq_s.pub, 800);
              } else {
                  mixHash(&(hs->symmetric_state), hs->pq_rs.pub, 800);
              }
          }
          break;
      case token_e:
          if(!pq) {
              ...
              if ((initiator && direction) || (!initiator && !direction)) {
                  mixHash(&(hs->symmetric_state), hs->pq_e.pub, 800);
              } else {
                  mixHash(&(hs->symmetric_state), hs->pq_re.pub, 800);
              }
          }
          break;
      case token_end_turn:
    ...
    switch (*current_token) {
      case token_e:
        if(!hs->is_pq) {
            ...
            assert(!hs->pq_e.isSet);
            generate_pqKeyPair(&(hs->pq_e), kem);
            memcpy(p, hs->pq_e.pub, 800);
            p += 800;
            mixHash(&(hs->symmetric_state), hs->pq_e.pub, 800);
        }
        break;
      case token_s:
          if(!hs->is_pq){
              ...
              assert(hs->pq_s.isSet);
              memcpy(p, hs->pq_s.pub, 800);
              encryptAndHash(&(hs->symmetric_state), p, 800);
              p += 800;
              if (hs->symmetric_state.isKeyed) {
                  p += 16;
              }
      ...
      case token_ss:
        ...
        break;
      case token_ekem:
          assert(hs->pq_re.isSet);
          kem->encaps(p, DH_result, hs->pq_re.pub);
          mixHash(&(hs->symmetric_state), p, 768);
          //memcpy(p, cipher, 768);
          p += 768;
          mixKey(&(hs->symmetric_state), DH_result);
          break;
      case token_skem:
          assert(hs->pq_rs.isSet);
          kem->encaps(p, DH_result, hs->pq_rs.pub);
          //memcpy(p, cipher, 768);
          encryptAndHash(&(hs->symmetric_state), p, 768);
          p += 768;
          if (hs->symmetric_state.isKeyed) {
              p += 16;
          }
    ...
    switch (*current_token) {
      case token_e:
        if(!hs->is_pq){
            ...
            assert(!hs->pq_re.isSet);
            memcpy(hs->pq_re.pub, message, 800);
            message_len -= 800;
            message += 800;
            hs->pq_re.isSet = true;
            mixHash(&(hs->symmetric_state), hs->pq_re.pub, 800);
      ...
      case token_s:
        if(!hs->is_pq){
            ...
            assert(!hs->pq_rs.isSet);
            /* Not 100% sure if that size is correct, should be since strobe encryption is a xor but idk*/
            size_t ciphertext_len = 800;
            if (hs->symmetric_state.isKeyed) {
                ciphertext_len += 16;
            }
            if (message_len < ciphertext_len) {
            ...
            memcpy(hs->pq_rs.pub, message, 800);
            message_len -= ciphertext_len;
            message += ciphertext_len;
            hs->pq_rs.isSet = true;
        }
      ...
      case token_ekem:
          ...
          assert(hs->pq_e.isSet);
          mixHash(&(hs->symmetric_state), message, 768);
          kem->decaps(DH_result, message, hs->pq_e.priv);
          message_len -= 768;
          message += 768;
          mixKey(&(hs->symmetric_state), DH_result);
          break;
      case token_skem:
          ...
          kem->decaps(DH_result, message, hs->pq_s.priv);
          message_len -= ciphertext_len;
          message += ciphertext_len;
          mixKey(&(hs->symmetric_state), DH_result);
          break;
