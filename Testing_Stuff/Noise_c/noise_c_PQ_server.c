	...
	snprintf(to_test[1], sizeof(to_test[1]), "pq%s", argv[1]);
   	...
   	server.sin_family = AF_INET;
		...
		comp_ms = 0;
			...
			err = noise_handshakestate_new_by_name(&handshake,  to_test_full[k], NOISE_ROLE_RESPONDER);
		...
		printf("%s & %7.2f & %7.2f & %7.2f & %7.2f & %7.2f & %7.2f \\\\ \n", to_test[k], overall_ms/test_number, results[test_number/2], results[test_number-1], results[0], (total_time_comp/test_number+1)/1000000.0, comp_ms/(test_number+1));
		if(k % 2 == 0){
			printf("\\hline \n");
		}
