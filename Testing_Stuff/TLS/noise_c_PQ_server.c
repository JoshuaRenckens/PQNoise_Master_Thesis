...
int main(int argc, char *argv[])
{
    ...
    while( (new_socket = accept(socket_desc, (struct sockaddr *)&client, (socklen_t*)&c)) )
    {
		...
		err = noise_handshakestate_new_by_name(&handshake,  "Noise_pqXN_Kyber512_ChaChaPoly_BLAKE2s", NOISE_ROLE_RESPONDER);
	 	if (err != NOISE_ERROR_NONE) {
		...
		int sent = send(new_socket, message, 800, 0);
		if (sent < 0) {
		    	puts("Error on receiving the public key");
		}*/
		
		/*Receive the clients static public key*/
		/*int rec = recv(new_socket, message, 800, 0);
		if (rec < 0) {
		    	puts("Error on receiving the public key");
		}
		...
		while (ok) {
			...
			if (action == NOISE_ACTION_WRITE_MESSAGE) {
			    ...
			    if (send(new_socket , message , mbuf.size + 2 , 0) < 0) {
